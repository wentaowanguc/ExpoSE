...
\Function{Search}{$i$}
	...
	\While{$pq$ not empty}
		\State $input \gets$ \Call{FrontPop}{$pq$}
		\State $inputs \gets$ \Call{ExpandInputs}{$input$}
		\While{$inputs$ not empty}
			\State $testInput \gets$ \Call{FrontPop}{$inputs$}
			\State \Call{SymbolicExecute}{$testInput$}
...
\begin{figure}[t]
\begin{algorithmic}
\Function{UnitPropagate}{$\mathit{F}$, $Model$}
...
\begin{figure}[t]
\begin{algorithmic}
\Function{PureLiteral}{$\mathit{F}$, $Model$}
...
\begin{figure}[t]
\begin{algorithmic}
\Function{DPLL}{$\mathit{F}$, $Model$}
	\State \Call{UnitPropagate}{$\mathit{F}$, $Model$}
	\If{$\mathit{F}$ contains an empty clause}
		\State return unsat
	\EndIf
	\State \Call{PureLiteral}{$\mathit{F}$, $Model$}
	\If{$\mathit{F} = T$}
		\State return sat with the model $Model$
	\EndIf
	\State $l \gets$ a literal containing an atom from $\mathit{F}$
	\State \Call{DPLL}{$\mathit{F}|L$, $Model \cup {L}$}
	\State \Call{DPLL}{$\mathit{F}|\lnot{L}$, $Model \cup {\lnot{L}}$}
\EndFunction    
\end{algorithmic}
...
There are some micro-progrms that make good individual test cases to run:
\begin{itemize}
